...
You can use `Ash.PlugHelpers.set_tenant/2` in your plug pipeline to set the tenant for all subsequent operations:

```elixir
conn
|> Ash.PlugHelpers.set_tenant(country_code)
```
...
Alternatively, you can use `Ash.Scope` to set the tenant at the beginning of your application or process. This is useful for setting tenant context globally:

```elixir
Ash.Scope.set_tenant(organization_id)
```

For more information on `Ash.Scope`, see the [Ash.Scope documentation](https://hexdocs.pm/ash/Ash.Scope.html).
